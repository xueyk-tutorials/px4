...
这个会启动gazebo仿真窗口并且具备四个无人机，每个无人机都会启动用于地面站gcs连接的mavlink和用于offboard连接的mavlink。连接默认IP和端口如下：
...
- gcs连接：打开QGC后，只需要监听本地14550端口即可完成多机连接；如果仿真与地面站不在同一个IP，则需要增加多个目标，例如在IP为172.26.134.252的计算机启动仿真，则地面站连接三个无人机的通信配置如下图：

  ![image-20240113221103006](imgs/image-20240113221103006.png)
